# CatalogRecommender — implicit-feedback ranking

An item-to-item recommender with cosine similarity, popularity fallback, explanations, and chronological holdout evaluation against a baseline.

**Focus:** Machine learning engineering / recommendation systems · Python 3.11+ · Standard library · Offline demo
...
| Live ranking | TF-IDF vectors and cosine similarity implemented in Python |
| Offline model | Item-to-item cosine similarity from implicit feedback |
| Evaluation | Temporal holdout, hit rate, MRR, popularity baseline |
| Live source | GitHub repository search API |
...
Fetches current machine-learning repository metadata from the [GitHub repository search API](https://docs.github.com/en/rest/search/search#search-repositories) and recommends repositories by TF-IDF cosine similarity to explicit interests. The live content model handles cold start; the separate offline collaborative model demonstrates historical-interaction evaluation. Search results are sampled and index freshness is controlled by GitHub.
...
- **12 automated tests passed** locally on Python 3.14.
...
Gather opt-in relevance feedback and evaluate precision@k before claiming online recommendation quality.
